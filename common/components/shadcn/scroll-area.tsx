...
const ScrollArea = React.forwardRef<React.ElementRef<typeof ScrollAreaPrimitive.Root>, React.ComponentPropsWithoutRef<typeof ScrollAreaPrimitive.Root>>(({ className, children, ...props }, ref) => (
  <ScrollAreaPrimitive.Root data-sid="40667082" ref={ref} className={cn('relative overflow-hidden', className)} {...props}>
  ...
  ({ className, orientation = 'vertical', ...props }, ref) => (
    <ScrollAreaPrimitive.ScrollAreaScrollbar
      ...
      className={cn(
        ...
        orientation === 'vertical' && 'h-full w-10 border-l border-l-transparent p-[1px]',
        orientation === 'horizontal' && 'h-10 flex-col border-t border-t-transparent p-[1px]',
        className,
      )}
      {...props}>
