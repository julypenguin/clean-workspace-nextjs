...
// 體育
const OPT = {
  URL: 'https://sportsapidoc-2.cxsport.net/apidoc/%E4%BD%93%E8%82%B2%E6%8E%A5%E5%8F%A3%E6%96%87%E6%A1%A3/',
  DECLARE_PATH: path.resolve(path.dirname('.'), './common/@types/sports-apis.d.ts'),
  METHODS_PATH: path.resolve(path.dirname('.'), './common/apis/index.ts'),
}
...
const createMethods = async (apis) => {
  ...
  apis.forEach((api) => {
    const { pathname } = new URL(api.url)
    // 網址上的參數
    const isDynamicPath = MatchDynamicPath.test(api.url)
    // 方法名
    ...
    if (method === 'post') {
      if (isDynamicPath) {
        ...
        func += [
          ...
          `return (this?.instance || SportAPI).post(\`${decodeURIComponent(pathname).replace(/\{/g, '${').replace(/\/$/, '')}\`${params ? ', params' : ', null'}, this || undefined);`,
          `}\n`,
        ].join('\n')
      } else {
        func += [
          `export const req${name} = async function (this: AxiosRequestConfig | void${params}): Promise<${response}> {`,
          `return (this?.instance || SportAPI).post("${pathname.replace(/\/$/, '')}"${params ? ', params' : ', null'}, this || undefined);`,
          `}\n`,
        ].join('\n')
      }
    } else if (method === 'get') {
      if (isDynamicPath) {
        ...
        func += [
          `export const req${name} = async function (this: AxiosRequestConfig | void${params}): Promise<${response}> {`,
          `const { ${dynamicParams.join(',')} } = params || {}`,
          `return (this?.instance || SportAPI).get(\`${decodeURIComponent(pathname).replace(/\{/g, '${').replace(/\/$/, '')}\`, ${params ? '{ ...this, params }' : 'this || undefined'});`,
          `}\n`,
        ].join('\n')
      } else {
        func += [
          `export const req${name} = async function (this: AxiosRequestConfig | void${params}): Promise<${response}> {`,
          `return (this?.instance || SportAPI).get("${pathname.replace(/\/$/, '')}", ${params ? '{ ...this, params }' : 'this || undefined'});`,
          `}\n`,
        ].join('\n')
      }
    }
...
try {
  ...
  // 結束
  console.log('SportAPI.d.ts 建構完成')
} catch (error) {
